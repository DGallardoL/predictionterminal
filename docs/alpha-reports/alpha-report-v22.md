...
> **Two metrics, both reported.** "Joint days" below is the **calendar span** of overlapping contract coverage (first joint observation → last, including data-gap days where one leg had no print). The implementation gate in `api/src/pfm/alpha_tier_regen.py` (`JOINT_DAYS_4Q_GATE = 360`) is checked against `res.n_obs` — the count of **aligned non-NaN trading data points** after the inner-join. The two metrics relate as `n_obs ≈ 0.71 × joint_calendar_days` across the 5 pairs below. Both are stored on each JSON record as `n_obs` and `joint_calendar_days_v22` respectively. Neither metric reaches the 360-threshold for the 5 pairs below, so the verdict is the same under either reading; we mention this only so future-you doesn't read "174 days" and find n_obs=123 in the JSON and worry the data drifted.

| Pair | Joint days (cal) | n_obs (aligned) | Q1 | Q2 | Q3 | Q4 | Verdict |
|---|---:|---:|---:|---:|---:|---:|---|
| `clmence_guett / tom_steyer` | 133 | 91 | 2.61 | 2.96 | INSUF | INSUF | **NO_DATA → revert** |
| `fed_target_45_eoy / no_fed_cuts_2026` | 126 | 76 | 0.00 (no signal) | INSUF | INSUF | INSUF | **NO_DATA → revert** |
| `asselineau / palomino` | 153 | 107 | 0.00 (no signal) | 3.24 | INSUF | INSUF | **FAIL → revert** (Q1 sh=0) |
| `renan_santos / us_aliens` | 174 | 123 | 3.42 | 3.14 | 6.26 | INSUF | **NO_DATA, but lenient 3/3 = B_VALIDATED++** |
| `grenell / us_iran_deal_jun` | 134 | 92 | 4.52 | 2.72 | INSUF | INSUF | **NO_DATA → revert** |
