...
class ArbCandidate:
    ...
    confidence: str = "?"  # "HIGH" | "MED" | "LOW" | "?" (from event config)
...
ROLL_CACHE_FILE = "roll_spread_cache.json"
VPIN_STATUS_FILE = "vpin_status.json"


def load_blacklist() -> set:
    """Load the union of (a) hand-blacklisted arb_keys and (b) the Roll-spread
    blacklist computed by ``research/build_roll_filter.py``.

    The Roll blacklist contains pairs where Polymarket-leg implicit spread
    (Roll 1984) exceeds the displayed cross-venue gap — these are bid-ask
    bounce illusions, not real arbs.

    Wave-6 freshness gate: the Roll cache is IGNORED if older than 48h. The
    audit (2026-05-19) found that stale daily-max history surfaces regimes
    that no longer hold — 2 of 4 blacklist entries were old-regime false
    positives. Hourly fidelity + 48h freshness keeps the filter accurate.
    """
    keys: set[str] = set()
    try:
        ...
        pass
    try:
        with open(ROLL_CACHE_FILE, encoding="utf-8") as f:
            payload = json.load(f)
        ...
        try:
            ...
            is_fresh = age_h <= 48.0
            if not is_fresh:
                log.warning("Roll cache stale (%.1fh) — ignoring blacklist", age_h)
        except (ValueError, TypeError):
            pass
        if is_fresh:
            keys |= set(payload.get("blacklist_arb_keys", []) or [])
    except (FileNotFoundError, json.JSONDecodeError):
        pass
    return keys


def load_vpin_gate() -> dict:
    """Read VPIN_bulk status (computed by ``research/build_vpin_status.py``).

    Returns a dict with keys: vpin_bulk_now, level (low|warn|halt|unknown),
    as_of. When the file is missing or stale (>30 min), returns level=unknown
    so the engine doesn't gate alerts on stale data.
    """
    try:
        with open(VPIN_STATUS_FILE, encoding="utf-8") as f:
            data = json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        return {"level": "unknown", "reason": "no status file"}
    as_of = data.get("as_of", "")
    # Treat status older than 30 min as unknown — VPIN moves on minutes.
    try:
        ts = datetime.strptime(as_of, "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
        age = (datetime.now(timezone.utc) - ts).total_seconds()
        if age > 1800:
            return {"level": "unknown", "reason": f"stale ({age:.0f}s)", "as_of": as_of}
    except (ValueError, TypeError):
        pass
    return data
...
def write_dashboard_state(opportunities, bal_k, bal_p, config, elapsed,
    ...
    # Microstructure gates (Roll filter + VPIN_bulk toxicity) — exposed to UI
    # so users see when alerts are being suppressed and why.
    vpin = load_vpin_gate()
    try:
        with open(ROLL_CACHE_FILE, encoding="utf-8") as _f:
            roll_summary = json.load(_f).get("summary", {})
    except (FileNotFoundError, json.JSONDecodeError):
        roll_summary = {}
    ...
    state = {
        ...
        "microstructure": {
            "vpin_bulk_now": vpin.get("vpin_bulk_now"),
            "vpin_level": vpin.get("level", "unknown"),
            "vpin_warn_threshold": vpin.get("warn_threshold"),
            "vpin_halt_threshold": vpin.get("halt_threshold"),
            "vpin_as_of": vpin.get("as_of"),
...
class ArbScanner:
    ...
    def load_config(self) -> dict:
        ...
        seen_tickers = set()
        # Autoload of secondary configs: ON by default (was OFF post-2026-05-19
        # audit). User preference is volume-over-precision — auto-discovered
        # MED entries show up under the "Discovered" tab where the source tag
        # + confidence badge make FPs easy to spot. Set
        # PFM_ARB_INCLUDE_SECONDARY=0 to suppress.
        all_paths = list(self.config_files)
        if os.environ.get("PFM_ARB_INCLUDE_SECONDARY", "1") == "1":
    ...
    def _screen_candidates(self, results, threshold, blacklist) -> tuple[list[ArbCandidate], list[dict]]:
        ...
        for r in results:
            ...
            confidence = (ec.get('confidence') or '?').upper()
    ...
    def _report_and_execute(self, candidates: list[ArbCandidate], min_alert_profit: float):
        ...
        # VPIN_bulk gate (Easley-LdP-O'Hara 2012). The 24h empirical baseline
        # (research/s4_vpin_btc_test.py) showed VPIN_bulk predicts RV_{t+30m}
        # with β=+0.0028, t=+2.71, R²=5.8%. High VPIN ⇒ informed flow on one
        # side ⇒ adverse selection ⇒ pause execution.
        #   level=warn (≥0.45) → log alert but skip execution
        #   level=halt (≥0.50) → don't even alert
        vpin = load_vpin_gate()
        vpin_level = vpin.get("level", "unknown")
        if vpin_level == "halt":
            log.warning("[VPIN-HALT] BTC VPIN_bulk=%.3f ≥ %.2f — skipping all alerts this cycle",
                        vpin.get("vpin_bulk_now", 0), vpin.get("halt_threshold", 0.50))
        elif vpin_level == "warn":
            log.info("[VPIN-WARN] BTC VPIN_bulk=%.3f ≥ %.2f — alerting but skipping execution",
                     vpin.get("vpin_bulk_now", 0), vpin.get("warn_threshold", 0.45))
        ...
        for c in candidates:
            ...
            if c.est_profit >= min_alert_profit:
                # VPIN-halt: drop the alert+execute branch entirely on toxic flow.
                if vpin_level == "halt":
                    continue
                # Cooldown check
                ...
                # VPIN-warn: send the alert (above) but skip the execution branch
                # so we don't take adverse-selection risk during high-toxicity windows.
                if vpin_level == "warn":
                    continue
    ...
    def run(self):
        ...
        self.notifier.start()
        # Auto-discovery spawns a heavy auto_discover.py subprocess every 30 min
        # that contends with the scan loop for ~4 min, letting dashboard_state.json
        # go stale (>3 min) so the API flips to fallback. Opt out to keep the scan
        # loop continuous and the dashboard reliably "engine LIVE".
        if os.getenv("PFM_ARB_NO_DISCOVERY") != "1":
            self._start_auto_discovery()
        else:
            log.info("[Discovery] disabled (PFM_ARB_NO_DISCOVERY=1) — continuous scanning")
    ...
    def _scan_cycle(self):
        ...
        config = self.load_config()
        events = config.get('events', [])
        # The config ships poll_interval=240s, but the API marks the dashboard
        # state stale after 180s → it flips to fallback during the sleep. Allow
        # an env override so the write cadence stays under the staleness window.
        poll_interval = int(os.getenv("PFM_ARB_POLL_INTERVAL", config.get('poll_interval', 8)))
